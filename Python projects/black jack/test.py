...
def dealer_draw(user_total, dealer_hand):
    if user_total < 21:
        while sum(dealer_hand) < 17:
            dealer_hand.append(draw())
    return dealer_hand

def blackjack(dealer_hand, user_hand): # game fuction
    ...
    dealer_draw(sum(user_hand), dealer_hand) # Function to draw card for dealer (if necessary)
...
def main():
    ...
    while play_game == False:
        ...
        if play == "y":
            blackjack(dealer_hand, user_hand)
            play_game = True
        elif play == "n":
